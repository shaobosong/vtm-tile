...
namespace netxs::app::parvion
{
    ...
    // width array (col_w), the column-model builder (queue_tbl), the renderer, and ctrl->col_shown.
    ...
    // Checksums-table columns (shared by queue_hash_tbl, its header show/hide menu, and ctrl->hash_col_shown).
    ...
    struct queue_state
    {
        ...
        si32                tab = 0;        // 0=Transferring, 1=Failed, 2=Succeeded, 3=Message log, 4=Checksums.
        bool                focused = faux;
        bool                hover = faux;   // Cursor is over the top handle bar (row 0).
        std::array<rect, 5> tabbox{};       // Cached tab hitboxes (bottom row).
        ...
        // Checksums-tab column widths once user-dragged (0 = auto-size to content): Source, Path,
        // Algorithm, Size, Progress, Result. The Checksums table reuses the same qtable component.
        std::array<si32, 6> hash_col_w{};
        si32                hover_border = -1; // Visible column index (into the active tab's qtable) whose resize border is hovered (-1 = none).
        si32                col_drag = -1;     // Visible column index being width-dragged (-1 = none).
    ...
    // Paint text into the cell [x, x+w) at row y, shifted left by `hscroll` and right-clipped to
    // `disp_w`; content wider than the cell is tail-truncated with '…' (fit_ellipsis). The shared
    // left-aligned cell primitive for every table (transfer tabs + Checksums).
    inline void qcell(auto& canvas, si32 x, si32 w, si32 y, view s, ui32 fg, ui32 bg, si32 hscroll, si32 disp_w)
    {
        auto sx   = x - hscroll;
    ...
    // Generic scrollable table component shared by the transfer tabs and the Checksums tab: a list
    // of visible columns (title / width / alignment) over a left content margin (gutter), with the
    // header, per-cell text (hscroll-clipped, tail-ellipsized, optional right-align) and the
    // draggable column dividers. Column geometry (col_x / border_cx / content_w) is the single
    // source of truth for both the renderer and the resize mouse handlers. Vertical/horizontal
    // scroll geometry stays in queue_state (queue_layout_core / queue_vsb / queue_hsb), shared by all tabs.
    struct qtable
    {
        struct column
        {
            ...
            bool resizable = true;
            si32 key = -1;       // Caller tag (the transfer logical-column index; Checksums column index).
        };
        std::vector<column> cols; // Visible columns, left to right (rebuilt each render by the caller).
        si32 left = 1;            // Content-x of the first column (width of the gutter before it).

        // --- Encapsulated per-tab column behavior (populated by the tab's builder; see queue_tbl /
        // queue_hash_tbl). The component derives the header show/hide menu, resize persistence, and
        // double-click auto-fit from these, so every table tab gets identical, complete behavior and
        // none can be silently misconfigured (the bug this interface prevents: a tab with no header
        // menu). `roster` lists ALL columns — shown and hidden — for the menu; the hooks are keyed by
        // a column's logical id (column::key, stable across show/hide and reorder).
        ...
        auto border_at(si32 content_x) const -> si32 { for (auto i = si32{}; i < (si32)cols.size(); ++i) if (border_cx(i) == content_x) return i; return -1; }

        // Paint column `i`'s cell at row y; reserves the trailing divider cell, tail-ellipsizes, and
        // right-aligns when the column requests it.
        void paint_cell(auto& canvas, si32 i, si32 y, view s, ui32 fg, ui32 bg, si32 hscroll, si32 disp_w) const
        {
            ...
            if (c.right)
            {
                ...
                qcell(canvas, cx + std::max(0, colw - tw), tw, y, t, fg, bg, hscroll, disp_w);
            }
            else qcell(canvas, cx, colw, y, s, fg, bg, hscroll, disp_w);
        }
        // Column-title header row, painted on the header surface.
        ...
        // The draggable │ dividers, extended from `top` down to (exclusive) `bottom`. The column at
        // visible index hover_idx / drag_idx brightens. Preserves each cell's bg.
    ...
    // Build a table's column show/hide menu from its roster: one kind::check toggle per column
    // (▣ shown / □ hidden, mirroring the message log's "Show detailed log"). `set_shown(key, on)`
    // writes the tab's backing visibility flag; `deface` repaints. At least one column always stays
    // visible. Rebuilt on every open so the ▣/□ reflect live state. The shared table component calls
    // this for EVERY table tab (q_context_menu), driven only by the column model — so no tab can be
    // wired without its header menu (the inconsistency this interface removes).
    inline auto build_columns_menu(std::vector<qtable::col_toggle> const& roster,
                                   std::function<void(si32, bool)> set_shown,
                                   std::function<void()> deface) -> std::vector<app::shared::menu::item>
    {
        namespace m = app::shared::menu;
        auto items = std::vector<m::item>{};
        auto shown = si32{};
        ...
        for (auto& c : roster)
        {
            ...
            {
                if (on && shown <= 1) return; // Hiding the last visible column is disallowed.
                set_shown(key, !on);
                deface();
            };
            items.push_back(std::move(row));
    ...
    // ---- Shared table interaction layer (right-click menus + selection) ----------------------------
    // The per-tab config supplies the menu content and selection behavior; the component does the
    // hit-testing, popup, and selection bookkeeping. A null callback disables that piece, so each tab
    // opts in to exactly the parts it needs (e.g. the Checksums tab has menus but no row selection).

    // Configurable right-click menus for one table tab. Any builder may be null (that menu is off).
    // Note: the column-header (show/hide) menu is NOT here — it is intrinsic to the table component,
    // generated from the column model (qtable::roster) by q_context_menu, so every tab gets it.
    struct qmenu_cfg
    {
        std::function<std::vector<app::shared::menu::item>(si32)> item;            // Per-row menu for the row's key (null = none).
        std::function<std::vector<app::shared::menu::item>()>     blank;           // Blank-body-area menu (null = none).
        std::function<void(si32)>                                 on_item_rclick;  // Selection sync for a right-clicked row (null = none).
        std::function<void()>                                     on_blank_rclick; // Selection reset on a blank right-click (null = none).
    };

    // The table row whose render-time hit-box contains (mx,my); -1 = none (header / blank / outside).
    inline auto q_row_at(queue_state const& st, si32 mx, si32 my) -> si32
    {
    ...
    // Right-click dispatch for a table tab: the column-header row opens the show/hide menu (intrinsic
    // to the table — built from `t`'s roster, so it exists for every tab); an item row syncs the
    // selection then opens the item menu; the blank body area resets the selection then opens the
    // blank menu. The per-tab item/blank steps are skipped when their callback is null.
    inline void q_context_menu(auto& boss, queue_state& st, si32 mx, si32 my, qtable const& t, qmenu_cfg const& cfg)
    {
        namespace m = app::shared::menu;
        auto at = twod{ mx, my };
        if (my == 1 && st.body_top > 1) // Column-header row.
        {
        ...
        if (my < st.body_top || my >= st.tab_row) return; // Outside the table body.
        ...
        {
            if (cfg.on_blank_rclick) cfg.on_blank_rclick();
            if (cfg.blank)           m::open_dropdown_popup(boss, cfg.blank(), faux, -1, at);
        }
    }

    // Transfer tabs' right-click config: item = Start/Pause/Remove/Pin (acts on the live selection);
    // blank = *All; selection-sync makes a right-clicked unselected row the sole selection, and a
    // blank right-click clears the selection. (The header show/hide menu is intrinsic to the table.)
    inline auto q_transfer_menu_cfg(queue_state& st, netxs::wptr<ui::base> panel_wp) -> qmenu_cfg
    {
        auto sp     = &st;
        auto deface = [panel_wp]{ if (auto p = panel_wp.lock()) p->base::deface(); };
        auto cfg    = qmenu_cfg{};
        cfg.item   = [sp, panel_wp](si32){ return build_queue_item_menu(sp->ctrl, sp->tab, panel_wp, sp->window_wp); };
        cfg.blank  = [sp, panel_wp]{ return build_queue_all_menu(sp->ctrl, sp->tab, panel_wp, sp->window_wp); };
        cfg.on_item_rclick = [sp, deface](si32 hit)
        {
            if (hit < 0 || hit >= (si32)sp->ctrl->queue.size()) return;
            if (!sp->ctrl->queue[(size_t)hit].selected) // Keep an existing multi-selection; else select just this row.
            {
                for (auto& it : sp->ctrl->queue) it.selected = faux;
                sp->ctrl->queue[(size_t)hit].selected = true;
                sp->sel_anchor = hit;
                deface();
            }
        };
        cfg.on_blank_rclick = [sp, deface]
        {
            auto any = faux;
            for (auto& it : sp->ctrl->queue) { any |= it.selected; it.selected = faux; }
            if (any) { sp->sel_anchor = -1; deface(); }
        };
        return cfg;
    }

    // Checksums tab's right-click config: item = Copy digest / Remove (+ Clear finished); blank =
    // Clear finished. (The header show/hide menu is intrinsic to the table — see q_context_menu.)
    inline auto q_hash_menu_cfg(queue_state& st, netxs::wptr<ui::base> panel_wp) -> qmenu_cfg
    {
        namespace m = app::shared::menu;
        auto sp     = &st;
        auto deface = [panel_wp]{ if (auto p = panel_wp.lock()) p->base::deface(); };
        auto clear_item = [sp, deface]
        {
            auto clr = m::item{ .alive = true, .label = "Clear finished" };
            clr.action = [sp, deface](hids&){ sp->ctrl->hash_clear_finished(); deface(); };
            return clr;
        };
        auto cfg = qmenu_cfg{};
        cfg.item = [sp, deface, clear_item](si32 hit) -> std::vector<m::item>
        {
            auto items = std::vector<m::item>{};
            if (hit >= 0 && hit < (si32)sp->ctrl->hash_queue.size())
            {
                auto& it     = sp->ctrl->hash_queue[(size_t)hit];
                ...
                items.push_back(std::move(copy));
                // Remove acts on the whole selection (on_item_rclick has already made the row part of it).
                auto n  = sp->ctrl->hash_selected_count();
                auto rm = m::item{ .alive = true, .label = n > 1 ? "Remove " + std::to_string(n) + " checksums" : text{ "Remove" } };
                rm.action = [sp, deface](hids&){ sp->ctrl->hash_remove_selected(); deface(); };
        ...
        // Right-clicking a row makes it the sole selection unless it's already part of a multi-selection;
        // a blank right-click clears the selection (mirrors the transfer tabs).
        cfg.on_item_rclick = [sp, deface](si32 hit)
        {
            if (hit < 0 || hit >= (si32)sp->ctrl->hash_queue.size()) return;
            if (!sp->ctrl->hash_queue[(size_t)hit].selected)
            {
                for (auto& it : sp->ctrl->hash_queue) it.selected = faux;
                sp->ctrl->hash_queue[(size_t)hit].selected = true;
                sp->sel_anchor = hit;
                deface();
            }
        };
        ...
        {
            auto any = faux;
            for (auto& it : sp->ctrl->hash_queue) { any |= it.selected; it.selected = faux; }
            if (any) { sp->sel_anchor = -1; deface(); }
        };
        return cfg;
    }

    // Configurable row-selection adapter for the shared table component. A tab provides access to its
    // selection state over a flat key space (a key = a selectable row identity, e.g. a queue index);
    // a tab with no selection simply doesn't build one. `disp`/`key_of_row` map display rows (which
    // include non-selectable subtask children) to keys for the rubber-band band.
    struct qsel_cfg
    {
        std::function<si32()>          key_count;   // Size of the key space (e.g. queue size).
        std::function<bool(si32)>      is_sel;      // key -> selected?
        std::function<void(si32, bool)> set_sel;    // key -> set selected.
        std::function<void()>          clear;       // Deselect everything.
        std::function<bool()>          any;         // Anything selected?
        std::function<bool(si32)>      in_scope;    // key -> selectable on the active tab (Shift-range filter).
        std::function<si32()>          disp;        // Number of display rows.
        std::function<si32(si32)>      key_of_row;  // Display row -> selectable key (-1 if not selectable).
    };

    // Press-selection on the row with key `hit`: Shift extends the range from the anchor (tab-filtered),
    // Ctrl toggles, a plain press selects only this row. Returns true (the caller defaces + dismisses).
    inline auto q_sel_press(queue_state& st, qsel_cfg const& s, si32 hit, bool ctl, bool shft) -> bool
    {
    ...
    // A plain press on the blank body area clears the selection. Returns true if anything changed.
    inline auto q_sel_clear_blank(queue_state& st, qsel_cfg const& s) -> bool
    {
        if (!s.any()) return faux;
        s.clear(); st.sel_anchor = -1; return true;
    }
    // Snapshot the current selection (keys) for a Ctrl+drag merge (captured before the press mutates it).
    inline void q_sel_snapshot(queue_state& st, qsel_cfg const& s)
    {
        st.drag_base.clear();
        for (auto k = si32{}; k < s.key_count(); ++k) if (s.is_sel(k)) st.drag_base.insert(k);
    }
    // Begin a rubber-band at display row `press_row`: a plain band replaces the selection, a Ctrl band
    // adds/removes the swept rows relative to the pre-press snapshot (add when the anchor was unselected).
    inline void q_rubber_begin(queue_state& st, qsel_cfg const& s, si32 press_row, bool ctl)
    {
    ...
    // Extend the live rubber-band to display row `cur_row` and re-apply the selection.
    inline void q_rubber_pull(queue_state& st, qsel_cfg const& s, si32 cur_row)
    {
        st.rubber_b = cur_row;
        ...
        else // Plain band: the swept span becomes the selection.
        {
            s.clear();
    ...
    // The transfer tabs' selection adapter: keys are queue indices; display rows come from build_rows
    // (a parent row's key is its queue index, a subtask child row is not selectable). The rows are
    // snapshotted once per event so the rubber-band band maps display rows to keys consistently.
    inline auto q_transfer_sel_cfg(queue_state& st) -> qsel_cfg
    {
        auto sp   = &st;
        auto rows = std::make_shared<std::vector<disp_row>>();
        build_rows(st, *rows);
        auto cfg = qsel_cfg{};
        cfg.key_count  = [sp]{ return (si32)sp->ctrl->queue.size(); };
        cfg.is_sel     = [sp](si32 k){ return k >= 0 && k < (si32)sp->ctrl->queue.size() && sp->ctrl->queue[(size_t)k].selected; };
        cfg.set_sel    = [sp](si32 k, bool v){ if (k >= 0 && k < (si32)sp->ctrl->queue.size()) sp->ctrl->queue[(size_t)k].selected = v; };
        cfg.clear      = [sp]{ for (auto& it : sp->ctrl->queue) it.selected = faux; };
        cfg.any        = [sp]{ for (auto& it : sp->ctrl->queue) if (it.selected) return true; return faux; };
        cfg.in_scope   = [sp](si32 k){ return k >= 0 && k < (si32)sp->ctrl->queue.size() && item_in_tab(*sp, sp->ctrl->queue[(size_t)k]); };
    ...
    // The Checksums tab's selection adapter: keys = hash-queue indices; the list is flat (one display
    // row per item, every row selectable), so key_of_row is the identity.
    inline auto q_hash_sel_cfg(queue_state& st) -> qsel_cfg
    {
        auto sp  = &st;
        auto cfg = qsel_cfg{};
        cfg.key_count  = [sp]{ return (si32)sp->ctrl->hash_queue.size(); };
        cfg.is_sel     = [sp](si32 k){ return k >= 0 && k < (si32)sp->ctrl->hash_queue.size() && sp->ctrl->hash_queue[(size_t)k].selected; };
        cfg.set_sel    = [sp](si32 k, bool v){ if (k >= 0 && k < (si32)sp->ctrl->hash_queue.size()) sp->ctrl->hash_queue[(size_t)k].selected = v; };
        cfg.clear      = [sp]{ for (auto& it : sp->ctrl->hash_queue) it.selected = faux; };
        cfg.any        = [sp]{ for (auto& it : sp->ctrl->hash_queue) if (it.selected) return true; return faux; };
        cfg.in_scope   = [sp](si32 k){ return k >= 0 && k < (si32)sp->ctrl->hash_queue.size(); };
        cfg.disp       = [sp]{ return (si32)sp->ctrl->hash_queue.size(); };
        cfg.key_of_row = [sp](si32 i){ return i >= 0 && i < (si32)sp->ctrl->hash_queue.size() ? i : -1; };
        return cfg;
    }
    // (q_table_has_sel / q_active_sel_cfg are defined with the rest of the table binding below, so the
    // tab classification lives in exactly one place — see q_active_view.)
    ...
    inline auto queue_col_content_w(queue_state const& st, si32 col) -> si32; // (defined below; used by the table's autofit hook)
    inline auto hash_col_content_w(queue_state const& st, si32 col) -> si32;  //

    // Build the active transfer tab's visible columns as a generic qtable (the shared table
    // component). Each column carries its logical index as `key`; the qtable's roster/hooks let the
    // component map a dragged column, an auto-fit, or a header-menu toggle back to col_w / col_shown /
    // the Failed-tab Reason override without any tab-specific code outside this builder.
    inline auto queue_tbl(queue_state& st) -> qtable
    {
        auto t = qtable{};
        t.left = q_name_x; // Gutter for the direction arrow (x=1) + expand button (x=3).
        if (!st.ctrl) return t;
        for (auto i = si32{}; i < q_ncol; ++i)
            if (st.ctrl->col_shown[(size_t)i])
                t.cols.push_back(qtable::column{ text{ q_headers[(size_t)i] }, st.col_w[(size_t)i], /*right*/ i >= 2 && i <= 4, /*resizable*/ true, /*key*/ i });
        if (st.tab == 1 && st.ctrl->col_shown[(size_t)q_ncol]) // Reason: Failed tab only, left-aligned.
            t.cols.push_back(qtable::column{ text{ q_headers[(size_t)q_ncol] }, queue_reason_w(st), faux, true, q_ncol });
        // Header-menu roster: every column (shown + hidden). Reason is offered only on the Failed tab.
        auto n = st.tab == 1 ? q_ncol + 1 : q_ncol;
        for (auto i = si32{}; i < n; ++i)
            t.roster.push_back({ text{ q_headers[(size_t)i] }, i, st.ctrl->col_shown[(size_t)i] });
        // set_shown captures the stable ctrl pointer (it fires from the header-menu popup, which can
        // outlive this temporary qtable); resize/autofit fire synchronously from drag handlers.
        t.set_shown = [ctrl = st.ctrl](si32 key, bool on){ if (key >= 0 && key < (si32)ctrl->col_shown.size()) ctrl->col_shown[(size_t)key] = on; };
        t.resize    = [&st](si32 key, si32 w){ if (key < q_ncol) st.col_w[(size_t)key] = w; else st.reason_w_override = w; };
        t.autofit   = [&st](si32 key){ return queue_col_content_w(st, key); };
        return t;
    }

    // Checksums-tab columns: Source | Path | Algorithm | Size | Progress | Result, as the shared
    // qtable. Source / Path / Result auto-size to their widest content (each +1 for the trailing
    // divider) unless the user has dragged them (hash_col_w[i] > 0). key = column index.
    inline auto queue_hash_tbl(queue_state& st) -> qtable
    {
        ...
        for (auto& it : st.ctrl->hash_queue)
        {
        ...
        {
            if (!st.ctrl->hash_col_shown[(size_t)i]) continue; // Hidden via the header show/hide menu.
            auto w = st.hash_col_w[(size_t)i] > 0 ? st.hash_col_w[(size_t)i] : autow[(size_t)i];
            t.cols.push_back(qtable::column{ text{ hash_headers[(size_t)i] }, w, right[(size_t)i], true, i });
        }
        for (auto i = si32{}; i < 6; ++i) // Header-menu roster: every column (shown + hidden).
            t.roster.push_back({ text{ hash_headers[(size_t)i] }, i, st.ctrl->hash_col_shown[(size_t)i] });
        // set_shown captures the stable ctrl pointer (fires from the popup); resize/autofit are synchronous.
        t.set_shown = [ctrl = st.ctrl](si32 key, bool on){ if (key >= 0 && key < (si32)ctrl->hash_col_shown.size()) ctrl->hash_col_shown[(size_t)key] = on; };
        t.resize    = [&st](si32 key, si32 w){ if (key >= 0 && key < (si32)st.hash_col_w.size()) st.hash_col_w[(size_t)key] = w; };
        t.autofit   = [&st](si32 key){ return hash_col_content_w(st, key); };
        return t;
    }

    // Everything the shared table component needs to drive ONE tab, bound together in a single place
    // (q_active_view) so a tab can never be half-wired — the bug class this prevents is a tab that has,
    // say, a column model but no selection, or a menu but no header toggles. The members are lazy
    // factories: the column model is rebuilt every render/hover, but the menu and selection are built
    // only on the events that use them. A non-table tab (the Message log) leaves all three null; a tab
    // with no row selection leaves `build_sel` null — and q_table_has_sel derives from that, so there
    // is no second predicate to keep in sync.
    struct q_table_view
    {
        std::function<qtable()>                          build_table; // Column model (header menu / resize / autofit live on it).
        std::function<qmenu_cfg(netxs::wptr<ui::base>)>  build_menu;  // Right-click item/blank/selection-sync config.
        std::function<qsel_cfg()>                        build_sel;   // Row-selection adapter (null => tab isn't selectable).
        explicit operator bool() const { return (bool)build_table; } // Is the active tab a table at all?
    };

    // The SINGLE point that classifies the active tab and binds its behaviors. Adding a table tab means
    // adding one branch here; every consumer (column model, right-click menu, selection) flows from it.
    inline auto q_active_view(queue_state& st) -> q_table_view
    {
        auto sp = &st;
        auto tv = q_table_view{};
        if (st.tab == 4) // Checksums.
        {
            tv.build_table = [sp]{ return queue_hash_tbl(*sp); };
            tv.build_menu  = [sp](netxs::wptr<ui::base> w){ return q_hash_menu_cfg(*sp, w); };
            tv.build_sel   = [sp]{ return q_hash_sel_cfg(*sp); };
        }
        else if (st.tab <= 2) // Transferring / Failed / Succeeded.
        {
            tv.build_table = [sp]{ return queue_tbl(*sp); };
            tv.build_menu  = [sp](netxs::wptr<ui::base> w){ return q_transfer_menu_cfg(*sp, w); };
            tv.build_sel   = [sp]{ return q_transfer_sel_cfg(*sp); };
        }
        // tab == 3 (Message log): not a table — all factories stay null.
        return tv;
    }

    // The active tab's column model (the shared table component). Empty for the Message-log tab.
    inline auto q_active_tbl(queue_state& st) -> qtable
    {
        auto tv = q_active_view(st);
        return tv.build_table ? tv.build_table() : qtable{};
    }
    // Does the active tab support row selection? Derived from the binding (no separate tab predicate).
    inline auto q_table_has_sel(queue_state& st) -> bool { return (bool)q_active_view(st).build_sel; }
    // The active tab's selection adapter (only valid when q_table_has_sel(st)).
    inline auto q_active_sel_cfg(queue_state& st) -> qsel_cfg { return q_active_view(st).build_sel(); }
    // The visible (resizable) column whose right-edge resize border sits at screen-x `mx`, else -1.
    ...
    // Persist visible column `v`'s new width via the table's own resize hook (which routes the
    // column's logical key to the active tab's backing store — col_w / Reason override / hash_col_w).
    inline void q_set_col_w(qtable const& t, si32 v, si32 w)
    {
        if (v >= 0 && v < (si32)t.cols.size() && t.resize) t.resize(t.cols[(size_t)v].key, w);
    }
    // Auto-fit (widest-content) width for visible column `v` via the table's autofit hook. Takes the
    // SAME visible-index space as q_set_col_w, so resize and auto-fit handlers never juggle the
    // visible-vs-logical index distinction (q_border_hit yields a visible index).
    inline auto q_col_autofit(qtable const& t, si32 v) -> si32
    {
        return v >= 0 && v < (si32)t.cols.size() && t.autofit ? t.autofit(t.cols[(size_t)v].key) : 0;
    }
    ...
    // Display-cell width of the widest content in Checksums column `col` (0..5) across the hash
    // queue, floored by the header. Drives the Checksums double-click auto-fit (mirrors the
    // renderer's per-cell formatting in queue_render's tab-4 branch).
    inline auto hash_col_content_w(queue_state const& st, si32 col) -> si32
    {
        ...
        for (auto& it : st.ctrl->hash_queue)
        {
            ...
            switch (col)
            {
                ...
                case 4:  s = it.status == hash_item::queued    ? text{ "queued" }
                           : it.status == hash_item::hashing   ? (it.size > 0 ? pct(100.0 * (double)it.done / (double)it.size) : human_size(it.done))
                           : it.status == hash_item::succeeded ? text{ "100.00%" } : text{ "failed" }; break;
    ...
    // Clamp both scroll axes to the freshly-settled geometry. Shared by every scrollable tab.
    inline void q_clamp_scroll(queue_state& st)
    {
    ...
    // Finish a table tab's body: extend the column dividers down through the drawn rows (highlighting
    // the hovered/dragged border) and paint the scrollbars. Shared by the transfer tabs + Checksums.
    inline void q_table_finish(queue_state& st, qtable const& t, auto& canvas, si32 hscroll, si32 disp_w)
    {
        ...
        t.paint_dividers(canvas, 1, st.div_bottom, hscroll, disp_w, st.hover_border, st.col_drag);
    ...
    inline void queue_render(queue_state& st, auto& canvas, twod size)
    {
        ...
        auto n_hash = (si32)st.ctrl->hash_queue.size();
        ...
        else if (st.tab == 4) // Checksums: the shared table component over the hash queue.
        {
            auto& hq = st.ctrl->hash_queue;
            // Columns (Source | Path | Algorithm | Size | Progress | Result) come from the shared
            // builder, so the mouse handlers' column geometry / resize agree with the paint. Source
            // is the origin host (empty = local); Path is the full file path; Result is the digest /
            // failure reason. Auto-sized columns (or the user's dragged widths) scroll horizontally.
            auto t = queue_hash_tbl(st);
            auto content_w = hq.empty() ? 0 : t.content_w();

            queue_layout_core(st, w, h, (si32)hq.size(), content_w, /*body_top=*/2);
            if (st.follow) st.scroll = std::max(0, st.total - st.body_rows); // Tail-follow: pin to the bottom.
            q_clamp_scroll(st);

            auto hs    = st.hscroll;
            auto clipw = st.disp_w;
            auto pct_str = [](double pct)
            {
                auto buf = std::array<char, 24>{};
                std::snprintf(buf.data(), buf.size(), "%.2f%%", pct);
                return text{ buf.data() };
            };
            t.paint_header(canvas, 1, hs, clipw);

            if (hq.empty())
                put_str(canvas, 1, 2, "No checksums.", theme::subtext, theme::bg, std::max(1, st.disp_w - 1));
            auto first     = st.scroll;
            auto last      = std::min((si32)hq.size(), st.scroll + st.body_rows);
            auto rubber_on = st.drag == queue_state::d_rubber && st.rubber_a >= 0; // Flat list: display row == hash index.
            ...
            for (auto i = first; i < last; ++i)
            {
                auto& it = hq[(size_t)i];
                auto y   = st.body_top + (i - first);
                st.row_hit.push_back({ rect{{ 0, y }, { std::max(0, st.disp_w), 1 }}, i });
                // Selection highlight (mirrors the transfer table): a muted row fill ending at the
                // last column, with a blue left accent when the panel is focused.
                auto sel    = it.selected || (rubber_on && i >= rlo && i <= rhi);
                ...
                t.paint_cell(canvas, 0, y, it.source(), theme::subtext, row_bg, hs, clipw);          // Source "user@host:port" (empty = local).
                t.paint_cell(canvas, 1, y, it.path, theme::text_fg, row_bg, hs, clipw);              // Full path.
                t.paint_cell(canvas, 2, y, hash_algo_label(it.algo), theme::subtext, row_bg, hs, clipw);
                t.paint_cell(canvas, 3, y, it.size >= 0 ? human_size(it.size) : text{ "" }, theme::subtext, row_bg, hs, clipw);
                auto prog = text{}; auto pc = ui32{ theme::subtext };
                     if (it.status == hash_item::queued)    { prog = "queued"; }
                else if (it.status == hash_item::hashing)   { prog = it.size > 0 ? pct_str(100.0 * (double)it.done / (double)it.size) : human_size(it.done); pc = theme::text_fg; }
                else if (it.status == hash_item::succeeded) { prog = "100.00%"; pc = theme::dir_fg; }
                else                                        { prog = "failed";  pc = theme::err_fg; }
                t.paint_cell(canvas, 4, y, prog, pc, row_bg, hs, clipw);
                auto res = it.status == hash_item::succeeded ? it.digest
                         : it.status == hash_item::failed    ? it.error : text{};
                auto rc  = it.status == hash_item::failed ? ui32{ theme::err_fg } : ui32{ theme::text_fg };
                t.paint_cell(canvas, 5, y, res, rc, row_bg, hs, clipw);
            }
            q_table_finish(st, t, canvas, hs, clipw); // Draggable dividers (down through the rows) + scrollbars.
        }
        else // Transfers: a scrollable, resizable-column table over the active tab.
        {
            ...
            // Vertical scroll: when following, keep the active transfer in view — pinned to the
            // bottom when there is no active item (newest-at-bottom tail); then clamp both axes.
            auto maxscroll = std::max(0, st.total - st.body_rows);
            if (st.follow)
            {
            ...
            // The active tab's columns via the shared table component (geometry + header + per-cell
            // painting + dividers). `lx` maps a logical column index (0..q_ncol) to its visible slot
            // in `t.cols`, or -1 when hidden — t.paint_cell ignores a -1 slot, so call-sites with no
            // extra condition need no guard.
            auto t  = queue_tbl(st);
            auto lx = std::array<si32, q_ncol + 1>{}; lx.fill(-1);
            for (auto k = si32{}; k < (si32)t.cols.size(); ++k) lx[(size_t)t.cols[(size_t)k].key] = k;
            auto hs    = st.hscroll;
            auto clipw = st.disp_w; // Right clip: keep painted cells off the VSB column.
            // Header row 1 (titles + alignment come from the column model).
            if (st.body_top > 1) t.paint_header(canvas, 1, hs, clipw);
            ...
            else
            {
                ...
                for (auto i = first; i < last; ++i)
                {
                    ...
                    if (child == -1) // Parent task row.
                    {
                        ...
                        qcell(canvas, q_dir_x, 2, y, arrow, dir_fg, row_bg, hs, clipw);
                        ...
                        else qcell(canvas, q_exp_x, q_exp_w, y, " + ", theme::subtext, row_bg, hs, clipw);
                        // Local Name / Remote Name: the full path in each.
                        t.paint_cell(canvas, lx[0], y, it.local_path,  dir_fg, row_bg, hs, clipw);
                        t.paint_cell(canvas, lx[1], y, it.remote_path, dir_fg, row_bg, hs, clipw);
                        t.paint_cell(canvas, lx[2], y, human_size(it.size), theme::subtext, row_bg, hs, clipw);
                        ...
                        t.paint_cell(canvas, lx[3], y, prog, pc, row_bg, hs, clipw);
                        if (it.rate.speed > 0.0)
                            t.paint_cell(canvas, lx[4], y, human_size((si64)(it.rate.speed + 0.5)) + "/s", theme::subtext, row_bg, hs, clipw);
                        if (it.status == queue_item::failed && !it.error.empty())
                            t.paint_cell(canvas, lx[q_ncol], y, it.error, theme::err_fg, row_bg, hs, clipw);
                    ...
                    else // Expanded parallel-subtask child row.
                    {
                        ...
                        if (child < (si32)ranges.size())
                        {
                            ...
                            auto label = "Part " + std::to_string(child + 1) + "/" + std::to_string(it.chunk_count)
                                       + "  (" + human_size(start) + "–" + human_size(end) + ")";
                            t.paint_cell(canvas, lx[0], y, label, theme::subtext, row_bg, hs, clipw);
                            ...
                            if (!cprog.empty()) t.paint_cell(canvas, lx[3], y, cprog, cc, row_bg, hs, clipw);
                        }
                    }
                }
            }

            // Draggable column dividers (extended down through the visible rows, preserving each
            // cell's bg) + scrollbars, via the shared component.
            q_table_finish(st, t, canvas, hs, clipw);
        }
        ...
        auto labels = std::array<text, 5>{ "Transferring (" + std::to_string(n_xfer) + ")",
                                           "Failed (" + std::to_string(n_fail) + ")",
                                           "Succeeded (" + std::to_string(n_done) + ")",
                                           "Message log",
                                           "Checksums (" + std::to_string(n_hash) + ")" };
        auto x = si32{ 0 };
        for (auto i = si32{}; i < 5; ++i)
        {
    ...
    inline auto make_queue_panel(sftp_remote* ctrl, netxs::wptr<ui::fork> resize_target, netxs::wptr<ui::base> window_wp = {}) -> ui::sptr
    {
        ...
        panel->invoke([&](auto& boss)
        {
            ...
            boss.on(tier::mouserelease, input::key::LeftDown, [&](hids& gear)
            {
                ...
                if (st.ctrl && q_table_has_sel(st)) q_sel_snapshot(st, q_active_sel_cfg(st)); // Selectable tabs (transfer + Checksums).
                ...
                // A press on a column border (the resize handle, extended down the body) is left to
                // the drag handler; never let it fall through to row selection. Every table tab
                // (transfer + Checksums) has resizable columns via the shared component.
                if (st.tab != 3 && my >= 1 && my < st.div_bottom)
                    if (q_border_hit(q_active_tbl(st), mx, st.hscroll) >= 0) return;
                // Row selection (any selectable tab) via the shared component: plain = pick one; Ctrl =
                // toggle; Shift = range from the anchor; a plain press on the empty body clears the
                // selection (a Ctrl press keeps it so a Ctrl+drag from blank can add onto it).
                if (q_table_has_sel(st))
                {
                    auto ctl  = !!(gear.ctlstat & hids::anyCtrl);
                    auto shft = !!(gear.ctlstat & hids::anyShift);
                    auto sel  = q_active_sel_cfg(st);
                    auto hit  = q_row_at(st, mx, my);
                    if (hit >= 0)
                    {
                        q_sel_press(st, sel, hit, ctl, shft);
                        boss.base::deface();
                        gear.dismiss();
                        return;
                    }
                    if (!ctl && my >= st.body_top && my < st.tab_row && q_sel_clear_blank(st, sel))
                    {
                        boss.base::deface();
                        gear.dismiss();
                        return;
                ...
                if (my == tab_row) for (auto i = si32{}; i < 5; ++i)
                {
                    auto& b = st.tabbox[i];
            ...
            boss.on(tier::mouserelease, input::key::RightClick, [&](hids& gear)
            {
                ...
                // Every table tab routes its right-click through the shared component via its binding:
                // the column model drives the intrinsic header show/hide menu, and build_menu supplies
                // the item/blank/selection config (the only part the transfer and Checksums tabs differ
                // in). A non-table tab has no binding, so there is nothing to open.
                auto tv = q_active_view(st);
                if (tv) // build_table / build_menu are both set for any table tab.
                {
                    auto panel_wp = ptr::shadow(boss.This());
                    q_context_menu(boss, st, mx, my, tv.build_table(), tv.build_menu(panel_wp));
                }
                gear.dismiss();
            });
            ...
            boss.on(tier::mouserelease, input::key::MouseMove, [&](hids& gear)
            {
                ...
                auto nb = si32{ -1 };
                if (st.tab != 3 && my >= 1 && my < st.div_bottom) // Whole divider length (header + item rows), any table tab.
                    nb = q_border_hit(q_active_tbl(st), mx, st.hscroll);
            ...
            boss.LISTEN(tier::release, e2::form::drag::start::_<hids::buttons::left>, gear)
            {
                ...
                if (st.tab != 3 && py >= 1 && py < st.div_bottom) // Grab a column divider (any table tab).
                {
                    auto tbl = q_active_tbl(st);
                    auto v   = q_border_hit(tbl, px, st.hscroll);
                    if (v >= 0)
                    {
                        pro::focus::set(boss.This(), gear.id, solo::on);
                        st.col_drag = v;
                        // Seed the Failed-tab Reason width from its current auto-size so dragging it starts smooth.
                        if (st.tab == 1 && tbl.cols[(size_t)v].key == q_ncol && st.reason_w_override == 0) st.reason_w_override = queue_reason_w(st);
                        st.drag = queue_state::d_col;
                        boss.base::deface();
                        return;
                    }
                }
                if (q_table_has_sel(st) && py >= st.body_top && py < st.body_top + st.body_rows) // Rubber-band: any selectable tab.
                {
                    pro::focus::set(boss.This(), gear.id, solo::on);
                    // Anchor on the pressed display row WITHOUT clamping to the last item: a press in
                    // the blank area below the list anchors on a virtual row, so dragging back down
                    // past every item deselects them all. The component applies the Ctrl-merge.
                    q_rubber_begin(st, q_active_sel_cfg(st), st.scroll + (py - st.body_top), !!(gear.ctlstat & hids::anyCtrl));
                    boss.base::deface();
                    return;
                }
            };
            boss.LISTEN(tier::release, e2::form::drag::pull::_<hids::buttons::left>, gear)
            {
                ...
                switch (st.drag)
                {
                    ...
                    case queue_state::d_col:
                    {
                        ...
                        // Generic over the active tab's column model (transfer + Checksums).
                        auto tbl = q_active_tbl(st);
                        if (st.col_drag >= 0 && st.col_drag < (si32)tbl.cols.size())
                        {
                            auto left = tbl.col_x(st.col_drag); // Stable during the drag (sums only earlier columns).
                            auto w    = std::clamp(mx + st.hscroll - left + 1, q_col_min, q_col_max);
                            q_set_col_w(tbl, st.col_drag, w);
                            boss.base::deface();
                        }
                        break;
                    }
                    case queue_state::d_rubber:
                        ...
                        q_rubber_pull(st, q_active_sel_cfg(st), st.scroll + std::clamp(my - st.body_top, 0, std::max(0, st.body_rows - 1)));
                        boss.base::deface();
                        break;
                    default: break;
                }
            ...
            boss.on(tier::mouserelease, input::key::LeftDoubleClick, [&, log_span_select](hids& gear)
            {
                ...
                if (v >= 0)
                {
                    auto contentw = q_col_autofit(tbl, v); // Widest content for the active tab's column.
                    q_set_col_w(tbl, v, std::clamp(contentw + 1, q_col_min, q_col_max));
                    boss.base::deface();
                    gear.dismiss();
                }
            });
            ...
            boss.LISTEN(tier::preview, input::events::keybd::any, gear)
            {
                ...
                if (st.tab >= 3) return; // Navigation applies to the transfer tables only (not Message log / Checksums).
                auto maxv  = std::max(0, st.total - st.body_rows);
