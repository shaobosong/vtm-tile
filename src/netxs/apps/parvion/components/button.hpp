...
namespace netxs::app::parvion
{
    ...
    inline void button_render(button_state const& state, button_cfg const& cfg, auto& canvas, twod size)
    {
        ...
        put_str(canvas, x, y, label, pal.foreground, pal.background, std::max(0, size.x - x));
        if      (state.press) canvas.fill(rect{ {}, size }, [](cell& c){ c.xlight(2); });
