...
struct fxp_xfer {
    uint64_t offset, furthestdata, filesize, limit;
...
static struct fxp_xfer *xfer_init(struct fxp_handle *fh, uint64_t offset, uint64_t length)
{
    ...
    /*
     * Outstanding-bytes budget for download read requests. Sized to cover the
     * bandwidth-delay product on fat pipes (e.g. 1 Gbps at 100 ms RTT ~= 12 MB)
     * with headroom; the per-request size below caps the number of concurrent
     * FXP_READs at req_maxsize / per_request_size.
     */
    xfer->req_maxsize = 1048576*32;
    xfer->err = false;
    xfer->filesize = UINT64_MAX;
    xfer->furthestdata = 0;
    xfer->limit = (length == UINT64_MAX || UINT64_MAX - offset < length) ? UINT64_MAX : offset + length;
...
void xfer_download_queue(struct fxp_xfer *xfer)
{
    while (xfer->req_totalsize < xfer->req_maxsize &&
        ...
        /*
         * 32 KiB per FXP_READ — see FXP_READ_REQUEST_SIZE in sftp.h: the
         * interoperable maximum (some servers reject larger single reads).
         */
        rr->len = (xfer->limit - xfer->offset < FXP_READ_REQUEST_SIZE) ? (int)(xfer->limit - xfer->offset) : FXP_READ_REQUEST_SIZE;
...
struct fxp_xfer *xfer_download_init(struct fxp_handle *fh, uint64_t offset, uint64_t length)
{
    struct fxp_xfer *xfer = xfer_init(fh, offset, length);
...
{
    struct fxp_xfer *xfer = xfer_init(fh, offset, UINT64_MAX);
 ...
 */
int xfer_upload_gotpkt(struct fxp_xfer *xfer, struct sftp_packet *pktin, int *acked_bytes)
{
    struct sftp_request *rreq;
    ...
    /*
     * Only count bytes once the server has acknowledged the write.
     * Reporting at queue time over-counts when the connection is torn
     * down with writes still pending and leaves gaps in the destination
     * file on resume.
     */
    if (acked_bytes)
        *acked_bytes = rr_len;
