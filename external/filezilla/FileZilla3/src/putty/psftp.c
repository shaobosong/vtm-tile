...
const char *const appname = "PARVIONSFTP";
 ...
 */
int sftp_get_file(char *fname, char *outfname, bool restart, uint64_t start_offset, uint64_t length)
{
     ...
     */
    ret = 1;
    xfer = xfer_download_init(fh, offset, length);
...
int sftp_put_file(char *fname, char *outfname, int restart, uint64_t start_offset, uint64_t length, int initialize)
{
    struct fxp_handle *fh;
    struct fxp_xfer *xfer;
    struct sftp_packet *pktin;
...
//FIXME    PUT_PERMISSIONS(attrs, permissions);
    if (initialize) {
        /*
         * Chunk 0 of a parallel upload truncates/creates the target so the
         * other chunks can safely write into their absolute offsets.
         */
    ...
    if (restart) {
        ...
        req = fxp_open_send(outfname,
                            SSH_FXF_WRITE | SSH_FXF_CREAT | (length == UINT64_MAX ? SSH_FXF_TRUNC : 0),
                            &attrs);
    }
    pktin = sftp_wait_for_reply(req);
    fh = fxp_open_recv(pktin, req);
    ...
    /*
     * 32 KiB per FXP_WRITE — see FXP_WRITE_REQUEST_SIZE in sftp.h: the
     * interoperable maximum (some servers reject larger single requests).
     * Heap-allocated to keep it off the stack.
     */
    buffer = snewn(FXP_WRITE_REQUEST_SIZE, char);
    ...
    while ((!err && !eof) || !xfer_done(xfer)) {
        ...
        while (xfer_upload_ready(xfer) && !err && !eof) {
            ...
            int toread = FXP_WRITE_REQUEST_SIZE;
            if (remaining < (uint64_t)toread) {
            ...
            len = read_from_file(file, buffer, toread);
            if (len == -1) {
                ...
                remaining -= len;
                /*
                 * Yield to ack processing once enough writes are in flight.
                 * The threshold is a soft upper bound on per-iteration writes,
                 * not a hard window cap: the real flow control is the SSH
                 * channel window plus xfer_upload_ready (sendbuffer drained).
                 */
                if (pending_receive() >= FXP_PENDING_RECV_BREAK)
                    break;
            }
        }
        ...
        if (!xfer_done(xfer)) {
            int acked = 0;
            pktin = sftp_recv();
            ret = xfer_upload_gotpkt(xfer, pktin, &acked);
            if (ret <= 0) {
...
int sftp_general_get(struct sftp_command *cmd, int restart)
{
    ...
    ret = sftp_get_file(fname, outfname, restart, 0, UINT64_MAX);
    sfree(fname);
    return ret;
}
...
int sftp_general_put(struct sftp_command *cmd, int restart)
{
    ...
    ret = sftp_put_file(fname, outfname, restart, 0, UINT64_MAX, 0);
    sfree(outfname);
    return ret;
}
...
static bool parse_uint64_arg(char const* in, uint64_t* out)
{
    if (!in || !*in) {
        return false;
    }
...
static int sftp_cmd_parvion_get(struct sftp_command *cmd)
{
    if (!backend) {
        not_connected();
        return 0;
    ...
    if (cmd->nwords != 5) {
        fzprintf(sftpError, "parvion-get: expects remote file, local file, offset and length");
        return 0;
    }

    uint64_t offset, length;
    if (!parse_uint64_arg(cmd->words[3], &offset) || !parse_uint64_arg(cmd->words[4], &length)) {
        fzprintf(sftpError, "parvion-get: invalid offset or length");
        return 0;
    }

    char* fname = canonify(cmd->words[1], false);
    if (!fname) {
        fzprintf(sftpError, "%s: canonify: %s", cmd->words[1], fxp_error());
        return 0;
    }

    int const ret = sftp_get_file(fname, cmd->words[2], false, offset, length);
    sfree(fname);
    return ret;
}

static int sftp_cmd_parvion_put(struct sftp_command *cmd)
{
    if (!backend) {
        not_connected();
        return 0;
    ...
    if (cmd->nwords != 6) {
        fzprintf(sftpError, "parvion-put: expects local file, remote file, offset, length and init flag");
        return 0;
    }

    uint64_t offset, length;
    if (!parse_uint64_arg(cmd->words[3], &offset) || !parse_uint64_arg(cmd->words[4], &length)) {
        fzprintf(sftpError, "parvion-put: invalid offset or length");
        return 0;
    }
    int const initialize = atoi(cmd->words[5]) != 0;

    char* outfname = canonify(cmd->words[2], false);
    if (!outfname) {
        fzprintf(sftpError, "%s: canonify: %s", cmd->words[2], fxp_error());
        return 0;
    }

    int const ret = sftp_put_file(cmd->words[1], outfname, false, offset, length, initialize);
    sfree(outfname);
    return ret;
}
...
static struct sftp_cmd_lookup {
    ...
    {
        "parvion-get", sftp_cmd_parvion_get
    },
    {
...
{
    printf("Not meant for human consumption. Use Parvion instead.\n");
    cleanup_exit(1);
}
...
int psftp_main(int argc, char *argv[])
{
    ...
    fzprintf(sftpReply, "parvionSftp started, protocol_version=%d", FZSFTP_PROTOCOL_VERSION);
