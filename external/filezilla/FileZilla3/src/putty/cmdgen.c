/*
 * pvputtygen, based on FileZilla's fzputtygen (itself based on PuTTY's puttygen).
 *
 * Built into the multi-call vtm-tile binary (vtm-tile -r pvputtygen) and linked
 * alongside parvionsftp_be, which already provides the PuTTY globals (it owns
 * PUTTY_DO_GLOBALS in psftp.c) and the real platform stubs (x_get_default,
 * sk_cleanup, buildinfo_gtk_relevant). Those are therefore NOT redefined here, to
 * avoid duplicate-symbol link errors; only log_eventlog (defined nowhere else in
 * the backend) is kept as a local stub.
 */
...
int main(int argc, char **argv)
{
    ...
    printf("pvputtygen\n");
    printf("Based on FileZilla's fzputtygen / PuTTY's puttygen\n");
    ...
    printf("This program is used by Parvion and not intended to be used directly.\n");
